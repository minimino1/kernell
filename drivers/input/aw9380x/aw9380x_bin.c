...
static int aw_check_sum(struct aw_bin *bin, int bin_num)
{
	...
	check_sum = AW_CAP_GET_32_DATA(*(p_check_sum + 3), *(p_check_sum + 2),
...
static int aw_check_register_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	DBG("aw_bin_parse p_check_sum = %p\n", p_check_sum);
	parse_register_num = AW_CAP_GET_32_DATA(*(p_check_sum + 3), *(p_check_sum + 2),
					*(p_check_sum + 1), *(p_check_sum));
...
static int aw_check_dsp_reg_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	p_check_sum =
	    &(bin->info.data[(bin->header_info[bin_num].valid_data_addr)]);
	DBG("aw_bin_parse p_check_sum = %p\n", p_check_sum);
	parse_dsp_reg_num = AW_CAP_GET_32_DATA(*(p_check_sum + 7),
					*(p_check_sum + 6),
					*(p_check_sum + 5),
					*(p_check_sum + 4));
	bin->header_info[bin_num].reg_data_byte_len =
	    AW_CAP_GET_32_DATA(*(p_check_sum + 11), *(p_check_sum + 10),
	...
	bin->header_info[bin_num].download_addr =
	    AW_CAP_GET_32_DATA(*(p_check_sum + 3), *(p_check_sum + 2),
			*(p_check_sum + 1), *(p_check_sum));
...
static int aw_check_soc_app_num_v1(struct aw_bin *bin, int bin_num)
{
	...
	p_check_sum = &(bin->info.data[(bin->header_info[bin_num].valid_data_addr)]);
	DBG("aw_bin_parse p_check_sum = %p\n", p_check_sum);
	bin->header_info[bin_num].app_version = AW_CAP_GET_32_DATA(*(p_check_sum + 3),
							    *(p_check_sum + 2),
							    *(p_check_sum + 1),
							    *(p_check_sum));
	parse_soc_app_num = AW_CAP_GET_32_DATA(*(p_check_sum + 11), *(p_check_sum + 10),
					*(p_check_sum + 9), *(p_check_sum + 8));
	...
	bin->header_info[bin_num].download_addr =
	    AW_CAP_GET_32_DATA(*(p_check_sum + 7), *(p_check_sum + 6),
				      ...
				      struct aw_bin *bin)
{
	...
	if (!bin_serial_num) {
		bin_start_addr = AW_CAP_GET_32_DATA(*(bin->p_addr + 67),
					     *(bin->p_addr + 66),
					     *(bin->p_addr + 65),
					     *(bin->p_addr + 64));
...
static int aw_get_multi_bin_header_1_0_0(struct aw_bin *bin)
{
	...
	DBG("aw_bin_parse enter multi bin branch -- %s\n", __func__);
	bin_num = AW_CAP_GET_32_DATA(*(bin->p_addr + 63),
			      *(bin->p_addr + 62),
...
static int aw_parse_bin_header_1_0_0(struct aw_bin *bin)
{
	...
	bin_data_type = AW_CAP_GET_32_DATA(*(bin->p_addr + 11),
				    *(bin->p_addr + 10),
				    *(bin->p_addr + 9), *(bin->p_addr + 8));
...
static int aw_check_bin_header_version(struct aw_bin *bin)
{
	...
	header_version = AW_CAP_GET_32_DATA(*(bin->p_addr + 7), *(bin->p_addr + 6),
				     *(bin->p_addr + 5), *(bin->p_addr + 4));
