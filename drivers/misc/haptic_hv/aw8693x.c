...
static int aw8693x_get_f0(struct aw_haptic *aw_haptic)
{
	...
	aw_info("LCC only set f_pre, use dts drv_width");
	haptic_hv_i2c_writes(aw_haptic, AW8693X_REG_CONTCFG2, cont_config, AW_I2C_BYTE_ONE);
#else
	...
	for(i = 0; i < 2; i++) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8693X_REG_GLBRD5, &glb_state, AW_I2C_BYTE_ONE);
		aw_info("glb_state-0x3f=0x%02x", glb_state);
	}
...
static int aw869xx_get_gpp(struct aw_haptic *aw_haptic)
{
	return (int)aw_haptic->gpp;
}
...
static void aw8693x_get_bemf_peak(struct aw_haptic *aw_haptic, uint16_t *peak)
{
	...
	aw_info("peak[0]=0x%02X, peak[1]=0x%02X, peak[2]=0x%02X",
...
#ifdef AW8693X_LCC_ALGO
	val = of_property_read_u8(np, "aw8693x_cont_drv2_lvl",
				   &aw_haptic->info.cont_drv2_lvl);
	if (val != 0)
		aw_err("aw8693x_cont_drv2_lvl not found");

	val = of_property_read_u8(np, "aw8693x_cont_drv_width", &aw_haptic->info.cont_drv_width);
	if (val != 0)
		aw_err("aw8693x_cont_drv_width not found");
...
static void aw8693x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693X_REG_SYSCTRL5, reg_val, AW_I2C_BYTE_ONE);
		...
		/* Cont drv2 lvl */
#ifdef AW8693X_LCC_ALGO
   	aw_info("LCC use dts cont_drv2_lvl:%x", aw_haptic->info.cont_drv2_lvl);
#else
	/* Cont drv2 lvl */
...
#endif
#ifdef AW8693X_LCC_ALGO
	aw_haptic->vmax = AW_LCC_DEFAULT_BST_9V;
	aw_haptic->rtp_scale_coff[0] = 1;
	aw_haptic->rtp_scale_coff[1] = 1;
#else
	/* Get vmax */
...
#ifdef AW8693X_LCC_ALGO
    	/* cont_drv_width */
	if (!aw_haptic->info.cont_drv_width) {
		aw_err("aw_haptic->info.cont_drv_width = 0!");
	} else {
	    aw_info("cont_drv_width=0x%02X", aw_haptic->info.cont_drv_width);
	    haptic_hv_i2c_writes(aw_haptic, AW8693X_REG_CONTCFG3, &aw_haptic->info.cont_drv_width, AW_I2C_BYTE_ONE);
	}
#endif
				  ...
				  const char *buf, size_t count)
{
 ...
 static ssize_t rtp_scale_coff_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);

	return snprintf(buf, PAGE_SIZE, "%d, %d\n",
				aw_haptic->rtp_scale_coff[0], aw_haptic->rtp_scale_coff[1]);
}

static ssize_t rtp_scale_coff_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	uint32_t scale_coff[2] = {0};

	if (sscanf(buf, "%x %x", &scale_coff[0], &scale_coff[1]) == 2) {
		aw_haptic->rtp_scale_coff[0] = scale_coff[0];
		aw_haptic->rtp_scale_coff[1] = scale_coff[1];
	}
...
static ssize_t golden_param_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "0x%04X,0x%04X,0x%04X,0x%04X,0x%04X,0x%04X\n",
			aw_haptic->golden_param[0], aw_haptic->golden_param[1],
			aw_haptic->golden_param[2], aw_haptic->golden_param[3],
			aw_haptic->golden_param[4], aw_haptic->golden_param[5]);
}
				  ...
				  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "0x%04X,0x%04X,0x%04X,0x%04X,0x%04X,0x%04X",
			&peak[0], &peak[1], &peak[2], &peak[3], &peak[4], &peak[5]) == 6) {
		aw_haptic->golden_param[0] = peak[0];
		aw_haptic->golden_param[1] = peak[1];
		aw_haptic->golden_param[2] = peak[2];
		aw_haptic->golden_param[3] = peak[3];
		aw_haptic->golden_param[4] = peak[4];
		aw_haptic->golden_param[5] = peak[5];
	}
				  ...
				  const char *buf, size_t count)
{
	...
	if (val == 1) {
		...
		aw_haptic->sample_param[0] = aw_haptic->uefi_param[0];
		aw_haptic->sample_param[1] = aw_haptic->uefi_param[1];
		aw_haptic->sample_param[2] = aw_haptic->uefi_param[2];
		aw_haptic->sample_param[3] = aw_haptic->uefi_param[3];
		aw_haptic->sample_param[4] = aw_haptic->uefi_param[4];
		aw_haptic->sample_param[5] = aw_haptic->uefi_param[5];
	} else {
		aw_err("val err");
	}
	mutex_unlock(&aw_haptic->lock);
...
static ssize_t lk_f0_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	uint32_t lk_f0 = 0;

	lk_f0 = (aw_haptic->sample_param[3] + aw_haptic->sample_param[4]) / 2;

	return snprintf(buf, PAGE_SIZE, "%u\n",lk_f0);
}

static ssize_t lk_f0_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
...
static ssize_t gpp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
...
static ssize_t gpp_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
...
static ssize_t auto_brake_store(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	...
	mutex_lock(&aw_haptic->lock);
	if (val > 0) {
		aw_info("enable auto brake");
		aw_haptic->auto_brake = 1;
		aw8693x_auto_brake_mode(aw_haptic, true);
	} else if (val == 0) {
		aw_info("disable auto brake");
		aw_haptic->auto_brake = 0;
		aw8693x_auto_brake_mode(aw_haptic, false);
	}
	mutex_unlock(&aw_haptic->lock);
...
static ssize_t auto_brake_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	cdev_t *cdev = dev_get_drvdata(dev);
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic, vib_dev);
	ssize_t len = 0;

	len += snprintf(buf + len, PAGE_SIZE - len, "%d\n", aw_haptic->auto_brake);
...
static ssize_t d2s_gain_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	d2s_gain = aw8693x_select_d2s_gain(aw_haptic->info.d2s_gain);
	len += snprintf(buf + len, PAGE_SIZE - len, "d2s_gain = %u\n", d2s_gain);
				  ...
				  const char *buf, size_t count)
{
	...
	case 1:
		...
		break;
	case 2:
		aw_haptic->info.d2s_gain = 1;
		break;
	case 4:
		aw_haptic->info.d2s_gain = 2;
		break;
	case 8:
		aw_haptic->info.d2s_gain = 3;
		break;
	case 10:
		aw_haptic->info.d2s_gain = 4;
		break;
	case 16:
		aw_haptic->info.d2s_gain = 5;
		break;
	case 20:
		aw_haptic->info.d2s_gain = 6;
		break;
	case 40:
		aw_haptic->info.d2s_gain = 7;
		break;
	default:
...
static DEVICE_ATTR_RW(rtp_scale_coff);
static DEVICE_ATTR_RW(golden_param);
static DEVICE_ATTR_RW(sample_param);
static DEVICE_ATTR_RW(auto_brake);
static DEVICE_ATTR_RW(d2s_gain);
static DEVICE_ATTR_RW(lk_f0);
#endif
static DEVICE_ATTR_RW(gpp);
