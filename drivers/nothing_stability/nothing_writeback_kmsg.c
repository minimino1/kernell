...
struct block_device *get_target_partition_block_device(char* partition_label)
{
	...
	if (strcmp(partition_label, NT_KMSG_PARTITION_LABEL) == 0) {
		maj = NT_KMSG_MAJOR;
		min = NT_KMSG_MIN;
	} else if (strcmp(partition_label, NT_UEFI_PARTITION_LABEL) == 0) {
		maj = NT_UEFI_MAJOR;
		min = NT_UEFI_MIN;
	} else {
		NT_rkl_err_print("partition_label is %s unexpected\n", partition_label);
		return NULL;
	}
	...
	while (retry_wait_for_device > 0){
		...
		if (maj != MAJOR(devt) || min != MINOR(devt)) {
				NT_rkl_err_print("fail at %d", __LINE__);
				return NULL;
		}
		bdev = blkdev_get_by_dev(devt, FMODE_READ | FMODE_WRITE, NULL, NULL);
		if (!IS_ERR(bdev)) {
			if (strcmp(partition_label, bdev->bd_meta_info->volname) == 0) {
				NT_rkl_err_print("Get partition: %s\n", (char *)bdev->bd_meta_info->volname);
				return bdev;
			} else {
				NT_rkl_err_print("volname: %s, partiion: %s is not match\n",
					(char *)bdev->bd_meta_info->volname, partition_label);
			}
		} else {
...
int create_NT_device(void) {
	...
	nt_sys_dev_class = class_create("NT");
