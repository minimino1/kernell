...
static int tfa98xx_rcvspk_gpio = 0;
...
int __attribute__((weak)) mtk_spk_send_ipi_buf_to_dsp(void *data_buffer, uint32_t data_size)
{
	(void)data_buffer;
	(void)data_size;
    pr_err("tfa98xx: [%s]you need to define this func in other module!\n", __FUNCTION__);
	return 0;
}
...
static enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	if (trace_level & 8) {
		...
		dev_dbg(&tfa98xx->i2c->dev, "tfa98xx: tfa_dev_start(%d,%d) time = %lld us\n",
	...
	if ((err == tfa_error_ok) && (tfa98xx->set_mtp_cal)) {
		...
		if (err_cal != tfa_error_ok) {
			pr_err("tfa98xx: Error, setting calibration value in mtp, err=%d\n", err_cal);
		}
		else {
			tfa98xx->set_mtp_cal = false;
			pr_info("tfa98xx: Calibration value (%d) set in mtp\n",
				tfa98xx->cal_data);
		}
	}
...
static void __tfa98xx_inputdev_check_register(struct tfa98xx *tfa98xx, bool unregister)
{
	...
	if (tfa98xx->input)
		dev_info(tfa98xx->codec->dev, "tfa98xx: Input device already registered, skipping\n");
	else
	...
	loff_t *ppos)
{
	...
	if (tfa98xx->tfa == NULL) {
		pr_info("tfa98xx: [0x%x] dsp is not available\n", tfa98xx->i2c->addr);
		return -ENODEV;
	}
	...
	if (buffer == NULL) {
		pr_info("tfa98xx: [0x%x] can not allocate memory\n", tfa98xx->i2c->addr);
		return -ENOMEM;
	}
	...
	if (tfa98xx->tfa->is_probus_device) {
		if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
			...
			error = -ENODEV;
			pr_info("tfa98xx: receive data fail as DSP NOT work\n");
		}
	} else {
	...
	if (error != Tfa98xx_Error_Ok) {
		pr_info("tfa98xx: [0x%x] tfa_dsp_msg_read error: %d\n", tfa98xx->i2c->addr, error);
		kfree(buffer);
		return -EFAULT;
	}
	...
	size_t count, loff_t *ppos)
{
	...
	if (tfa98xx->tfa->is_probus_device) {
		...
		if (error != Tfa98xx_Error_Ok) {
			pr_info("tfa98xx: [0x%x] dsp_msg error: %d\n",
				tfa98xx->i2c->addr, error);
			err = -EIO;
		}
		mutex_unlock(&tfa98xx->dsp_lock);
		...
		if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
			...
			if (error != Tfa98xx_Error_Ok) {
				pr_info("tfa98xx: [0x%x] tfaContWriteFile error: %d\n",
					tfa98xx->i2c->addr, error);
				err = -EIO;
			}
		} else {
...
static int tfa98xx_dbgfs_otc_get(void *data, u64 *val)
{
	...
	if (value < 0) {
		pr_err("tfa98xx:[0x%x] Unable to check DSP access: %d\n", tfa98xx->i2c->addr, value);
		return -EIO;
	}

	*val = value;
	pr_info("tfa98xx: [0x%x] OTC : %d\n", tfa98xx->i2c->addr, value);
...
static int tfa98xx_dbgfs_otc_set(void *data, u64 val)
{
	...
	if (val != 0 && val != 1) {
		pr_err("tfa98xx:[0x%x] Unexpected value %llu\n", tfa98xx->i2c->addr, val);
		return -EINVAL;
	}
...
static int tfa98xx_dbgfs_mtpex_set(void *data, u64 val)
{
	...
	if (val != 0) {
		pr_err("tfa98xx:[0x%x] Can only clear MTPEX (0 value expected)\n", tfa98xx->i2c->addr);
		return -EINVAL;
	}
	...
	size_t count, loff_t *ppos)
{
	...
	if (ret == tfa_error_ok) {
		...
		if (cal_profile < 0) {
			pr_warn("tfa98xx:[0x%x] Calibration profile not found\n",
				tfa98xx->i2c->addr);
		}
	...
	if (ret) {
		pr_info("tfa98xx:[0x%x] Calibration start failed (%d)\n", tfa98xx->i2c->addr, ret);
		return -EIO;
	}
	else {
		pr_info("tfa98xx:[0x%x] Calibration started\n", tfa98xx->i2c->addr);
	}
	...
	loff_t *ppos)
{
	...
	if (ret) {
		ret = -EIO;
		pr_err("tfa98xx:[0x%x] MTP read failed\n", tfa98xx->i2c->addr);
		goto r_c_err;
	}
	...
	if (ret) {
		ret = -EIO;
		pr_err("tfa98xx:[0x%x] calibration failed\n", tfa98xx->i2c->addr);
		goto r_c_err;
	}
	...
	if (!str) {
		ret = -ENOMEM;
		pr_err("tfa98xx:[0x%x] memory allocation failed\n", tfa98xx->i2c->addr);
		goto r_c_err;
	}
	...
	pr_info("tfa98xx: [0x%x] calib_done: %s", tfa98xx->i2c->addr, str);
	...
	loff_t *ppos)
{
	...
	for (i = 0xFA; i <= 0xFE; i++)
	{
		...
		if (ret < 0)
		{
			dev_err(&i2c->dev, "tfa98xx:Failed to read data register: %d\n",
					ret);
			return -EIO;
		}
	}
	...
	loff_t *ppos)
{
	...
	pr_info("tfa98xx: [0x%x] dsp_state : %s\n", tfa98xx->i2c->addr, str);
	...
	size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, start_cmd, sizeof(start_cmd) - 1)) {
		pr_info("tfa98xx:[0x%x] Manual triggering of dsp start...\n", tfa98xx->i2c->addr);
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa98xx_tfa_start(tfa98xx, tfa98xx->profile, tfa98xx->vstep);
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_info("tfa98xx: [0x%x] tfa_dev_start complete: %d\n", tfa98xx->i2c->addr, ret);
	}
	else if (!strncmp(buf, stop_cmd, sizeof(stop_cmd) - 1)) {
		pr_info("tfa98xx:[0x%x] Manual triggering of dsp stop...\n", tfa98xx->i2c->addr);
		mutex_lock(&tfa98xx->dsp_lock);
		ret = tfa_dev_stop(tfa98xx->tfa);
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_info("tfa98xx: [0x%x] tfa_dev_stop complete: %d\n", tfa98xx->i2c->addr, ret);
	}
	else if (!strncmp(buf, mon_start_cmd, sizeof(mon_start_cmd) - 1)) {
		pr_info("tfa98xx:[0x%x] Manual start of monitor thread...\n", tfa98xx->i2c->addr);
		queue_delayed_work(tfa98xx->tfa98xx_wq,
			&tfa98xx->monitor_work, HZ);
	}
	else if (!strncmp(buf, mon_stop_cmd, sizeof(mon_stop_cmd) - 1)) {
		pr_info("tfa98xx:[0x%x] Manual stop of monitor thread...\n", tfa98xx->i2c->addr);
	...
	loff_t *ppos)
{
	...
	pr_info("tfa98xx: [0x%x] fw_state : %s", tfa98xx->i2c->addr, str);
		...
		if (profile == tfa98xx->profile) {
			...
			if (ready) {
				...
				if (err) {
					...
					pr_info("tfa98xx: Succesfully changed vstep index!\n");
					change = 1;
				}
			}
...
#endif
		pr_info("tfa98xx: %d: vstep:%d, (control value: %d) - profile %d\n",
	...
	struct snd_ctl_elem_info *uinfo)
{
	...
	pr_info("tfa98xx: vsteps count: %d [prof=%d]\n", tfacont_get_max_vstep(tfa98xx->tfa, profile),
		profile);
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (tfa98xx_container == NULL) {
		pr_err("tfa98xx: the firmware hasn't yet been loaded!!\n");
		return -EINVAL;
	}
	if (no_start != 0)
		return 0;
	...
	if ((new_profile < 0) || (new_profile >= profile_count)) {
		pr_err("tfa98xx: not existing profile (%d)\n", new_profile);
		return -EINVAL;
	}
	...
	pr_info("tfa98xx: selected container profile [%d -> %d]\n", cur_prof_idx, prof_idx);
	pr_info("tfa98xx: switch profile [%s -> %s]\n",
		tfa_cont_profile_name(tfa98xx, cur_prof_idx),
		tfa_cont_profile_name(tfa98xx, prof_idx));
	if (gpio_is_valid(tfa98xx_rcvspk_gpio)) {
		if (strstr(tfa_cont_profile_name(tfa98xx, prof_idx), "receiver") != NULL) {
			gpio_direction_output(tfa98xx_rcvspk_gpio, 0);
			pr_info("tfa98xx: selected receiver rcvspk [%d]\n", gpio_get_value(tfa98xx_rcvspk_gpio));
		}else{
			gpio_direction_output(tfa98xx_rcvspk_gpio, 1);
			pr_info("tfa98xx: selected spk/music rcvspk [%d]\n", gpio_get_value(tfa98xx_rcvspk_gpio));
		}
	}
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
        ...
        /* set profile dynamically when the PA is working. */
		if (tfa98xx->pstream == 1) {
			...
			if (strstr(tfa_cont_profile_name(tfa98xx, cur_prof_idx), ".standby") != NULL) {
				pr_info("tfa98xx: Force to start at exiting from standby: [%d -> %d]\n",
					cur_prof_idx, prof_idx);
				ready = 1;
			}
			if (ready) {
				...
				if (err) {
					pr_info("tfa98xx: Write profile error: %d\n", err);
				}
				else {
					pr_info("tfa98xx: Changed to profile %d (vstep = %d)\n",
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE && 0 == send_flag) {
			...
			pr_info("tfa98xx: send data to DSP\n");
		} else if(0 == send_flag){
			error = -1;
			pr_info("tfa98xx: send data fail as DSP NOT work\n");
		}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("tfa98xx: set_fade_ctl value: %ld\n", ucontrol->value.integer.value[0]);
...
{
	struct tfa98xx *tfa98xx = NULL;
    int min = 1; // -1 means otp devices.

	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
        if (!tfa98xx->tfa->is_otp_device)
            min = 0; // 0 means need to save mtp after calibration.
    }
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		valume = (uint16_t)ucontrol->value.integer.value[i];
        pr_info("tfa98xx: PA-0x%x's Calibration value is %d\n",
            tfa98xx->tfa->slave_address, valume);
        if (valume == tfa98xx->cal_data)
            continue;
        tfa98xx->cal_data = valume;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		mutex_lock(&tfa98xx->dsp_lock);
        if (tfa98xx->tfa->is_otp_device)
            ucontrol->value.integer.value[tfa98xx->tfa->dev_idx] = tfa98xx->cal_data;
        else
...
static uint32_t tfa98xx_pcm_id;
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	/* Create a mixer item for set & get RE25C control,also support OTP devices.
       if min=-1, means is_otp. */
	name = devm_kzalloc(tfa98xx->codec->dev, MAX_CONTROL_NAME, GFP_KERNEL);
	if (!name)
		return -ENOMEM;
...
retry:
	...
	if (ret < 0) {
		pr_warn("tfa98xx: i2c error, retries left: %d\n", retries);
		if (retries) {
			retries--;
...
retry:
	...
	if (ret < 0) {
		pr_warn("tfa98xx: i2c error at subaddress 0x%x, retries left: %d\n", subaddress, retries);
		if (retries) {
			retries--;
			msleep(I2C_RETRY_DELAY);
			goto retry;
	...
	tfa98xx_write_dsp(struct tfa_device *tfa, int num_bytes, const char *command_buffer)
{
    if (tfa->is_probus_device) {
    	...
    	pr_info("tfa98xx: tfa98xx_write_dsp num_bytes %d\n", num_bytes);
    	...
    	if (err) {
    		pr_err("tfa98xx: dsp_msg error: err %d\n", err);
    	}
    	mdelay(20);
    	...
    	pr_info("tfa98xx: tfa98xx_write_dsp not define!\n");
    	return Tfa98xx_Error_Not_Supported;
    }
}
	...
	tfa98xx_read_dsp(struct tfa_device *tfa, int num_bytes, unsigned char *result_buffer)
{
    if (tfa->is_probus_device) {
    	...
    	pr_info("tfa98xx: tfa98xx_read_dsp num_bytes %d\n", num_bytes);

    	buffer = kmalloc(num_bytes, GFP_KERNEL);
    	if (buffer == NULL) {
    		pr_err("tfa98xx: tfa98xx_read_dsp can not allocate memory\n");
    		return -ENOMEM;
    	}

    	error = tfa98xx_receive_data_from_dsp(buffer, num_bytes, &DataLength);
    	if (error != 0) {
    		pr_info("tfa98xx: [0x%x] dsp_msg_read error: %d\n", tfa->slave_address, error);
    		kfree(buffer);
    		return -EFAULT;
    	}
    	...
    	pr_info("tfa98xx: tfa98xx_read_dsp not define!\n");
    	return Tfa98xx_Error_Not_Supported;
    }
}
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	pr_info("tfa98xx: loaded %s - size: %zu\n", fw_name, cont->size);
	...
	if (tfa98xx_container == NULL) {
		...
		pr_info("tfa98xx %.2s%.2s\n", container->version, container->subversion);
		pr_info("tfa98xx %.8s\n", container->customer);
		pr_info("tfa98xx %.8s\n", container->application);
		pr_info("tfa98xx %.8s\n", container->type);
		pr_info("tfa98xx %d ndev\n", container->ndev);
		pr_info("tfa98xx %d nprof\n", container->nprof);

		tfa_err = tfa_load_cnt(container, container_size);
		if (tfa_err != tfa_error_ok) {
			mutex_unlock(&tfa98xx_mutex);
			kfree(container);
			dev_err(tfa98xx->dev, "tfa98xx: Cannot load container file, aborting\n");
			return;
		}
		...
		pr_info("tfa98xx container file already loaded...\n");
	...
	*/
	...
	if (tfa98xx->tfa->dev_idx < 0) {
		dev_err(tfa98xx->dev, "tfa98xx: Failed to find TFA98xx @ 0x%.2x in container file\n", tfa98xx->i2c->addr);
		return;
	}
	...
	if (tfa_is_cold(tfa98xx->tfa) == 0) {
		pr_info("tfa98xx: Warning: device 0x%.2x is still warm\n", tfa98xx->i2c->addr);
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		pr_info("tfa98xx: Skipping tfa_dev_start (no FW: %d)\n", tfa98xx->dsp_fw_state);
		return;
	}
...
static int tfa98xx_fade_thread(void *data)
{
	...
	for(times = 1; times < g_step; times++) {
	    if (mute_status ==1) {
	        pr_info("tfa98xx: mute=1,stop fade thread");
	        break;
	    } else {
	        pr_info("tfa98xx: tfa98xx_send_volume %d times =%d",tfa98xx_volume_tab[times],times);
	...
	pr_info("tfa98xx: success stop fade thread");
...
static int tfa98xx_fade_task(void)
{
	...
	fade_thrd = kthread_run(tfa98xx_fade_thread,
			NULL, "tfa98xx-fade");
	if (IS_ERR_OR_NULL(fade_thrd)) {
		pr_err("tfa98xx: Failed to create fade thread:%ld",
				PTR_ERR(fade_thrd));
		return -EFAULT;
	}

	pr_info("tfa98xx: success fade thread");
	return 0;
}
	...
	struct snd_soc_dai *dai)
{
	...
	pr_info("tfa98xx: Requested rate: %d, sample size: %d, physical size: %d\n",
	...
	if (no_start != 0)
		return 0;
	/* set TDM bit width */
	pr_info("tfa98xx: %s: Requested width: %d\n", __func__,
			params_width(params));
	...
	pr_info("tfa98xx: mixer profile:container profile = [%d:%d]\n", tfa98xx_mixer_profile, prof_idx);
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values(void)
{
	...
	if ((tfa98xx_device_count > 1) && (tfa98xx_device_count == bytes[0])) {
		pr_info("tfa98xx: The calibration value was sent to host DSP.\n");
		bytes[0] = 0;
		return Tfa98xx_Error_Ok;
	}
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		...
		if (TFA_GET_BF(tfa, MTPEX) == 1) {
			...
			pr_info("tfa98xx: Device 0x%x cal value is 0x%x\n",
	...
	pr_info("tfa98xx: tfa98xx_device_count=%d  bytes[0]=%d\n",
	...
	if (tfa98xx_device_count == bytes[0]) {
		...
		pr_err("tfa98xx: load calibration data from device failed.\n");
...
enum Tfa98xx_Error tfa98xx_adsp_send_calib_values_from_hal(void)
{
	...
	*/
	...
	if (ret) {
		pr_err("tfa98xx: send data to error: err %d\n", ret);
	}
...
static int tfa98xx_send_volume(uint8_t volume, int only_left)
{
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
        ...
        pr_info("tfa98xx: send volume 0x%x to PA-0x%x.\n",  volume, tfa98xx->i2c->addr);
    ...
    if (is_probus) {
        ...
        pr_info("tfa98xx: tfadsp send volume 0x%x to host DSP.\n",  volume);
...
static int tfa98xx_send_mute_cmd(void)
{
	...
	pr_info("tfa98xx: send mute command to host DSP.\n");
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (no_start) {
		pr_info("tfa98xx: no_start parameter set no tfa_dev_start or tfa_dev_stop, returning\n");
		return 0;
	}
	...
	if (mute) {
        ...
        if (tfa98xx->tfa->is_probus_device) {
    		if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
    			...
    			pr_info("tfa98xx: Mute Fail as DSP NOT work\n");
    		}
        }
		...
		if (stream == SNDRV_PCM_STREAM_PLAYBACK)
		{
    		...
    		if (tfa98xx->tfa->is_probus_device) {
                if (tfa98xx->tfa->is_otp_device) {
        			pr_info("tfa98xx: send_calib_values from HAL\n");
        			tfa98xx_adsp_send_calib_values_from_hal();
                } else {
        			pr_info("tfa98xx: send_calib_values from MTP\n");
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	pr_info("tfa98xx: Container loading requested: %d\n", ret);
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	dev_dbg(dev, "tfa98xx_parse_dt rcvspk-gpio \n");
	tfa98xx->rcvspk_gpio = of_get_named_gpio(np, "rcvspk-gpio", 0);
	if (tfa98xx->rcvspk_gpio < 0)
		dev_err(dev, "No rcvspk switch gpio provided.\n");
	else
		dev_dbg(dev, "tfa98xx rcvspk switch gpio found: %d\n", tfa98xx->rcvspk_gpio);
	if (tfa98xx->rcvspk_gpio > 0) {
		tfa98xx_rcvspk_gpio = tfa98xx->rcvspk_gpio;
		gpio_request(tfa98xx_rcvspk_gpio, "rcvspk_gpio");
		gpio_direction_output(tfa98xx_rcvspk_gpio, 1);
		dev_dbg(dev, "tfa98xx_rcvspk_gpio: %d value:%d\n", tfa98xx_rcvspk_gpio,
			gpio_get_value(tfa98xx_rcvspk_gpio));
	}
	dev_dbg(dev, "tfa98xx_parse_dt rcvspk-gpio end\n");
	return 0;
}
	...
	char *buf, loff_t off, size_t count)
{
	...
	if (count != 1) {
		pr_info("tfa98xx invalid register address");
		return -EINVAL;
	}
	...
	char *buf, loff_t off, size_t count)
{
	...
	if (count == 0)
	{
		pr_info("tfa98xx: err !count is 0\n");
		return 0;
	}
	...
	pr_info("tfa98xx: P cal data is: 0x%x\n", tfa98xx_cali_l);
	pr_info("tfa98xx: S cal data is: 0x%x\n", tfa98xx_cali_r);
...
/*
	...
	if (tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
		...
		pr_info("tfa98xx P cal data is: %x%x%x\n", cali_data[3],cali_data[4],cali_data[5]);
		pr_info("tfa98xx S cal data is: %x%x%x\n", cali_data[6],cali_data[7],cali_data[8]);
	} else {
	...
	char *buf, loff_t off, size_t count)
{
	...
	if (count == 0 || count > 33)
	{
		pr_info("tfa98xx: err !count is 0 or > 33\n");
		return 0;
	}
/*
	volume_data = kmalloc(count, GFP_KERNEL);
	if (data == NULL) {
		pr_info("tfa98xx can not allocate memory\n");
		return  -ENOMEM;
	}
*/
...
#endif
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		case 0x96:
			...
			break;
		default:
			pr_info("tfa98xx: Unsupported device revision (0x%x)\n", reg & 0xff);
			return -EINVAL;
		}
	}
#ifdef TFA9894_STEREO
