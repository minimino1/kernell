...
void tfaContShowHeader(TfaHeader_t *hdr) {
	...
	pr_info("tfa98xx File header\n");

	_id[1] = hdr->id >> 8;
	_id[0] = hdr->id & 0xff;
	pr_info("tfa98xx \tid:%.2s version:%.2s subversion:%.2s\n", _id,
		hdr->version, hdr->subversion);
	pr_info("tfa98xx \tsize:%d CRC:0x%08x \n", hdr->size, hdr->CRC);
	pr_info("tfa98xx \tcustomer:%.8s application:%.8s type:%.8s\n", hdr->customer,
...
TfaFileDsc_t *tfacont_getfiledata(struct tfa_device *tfa, int prof_idx, enum TfaHeaderType type)
{
	...
	if (tfa->verbose)
		pr_info("tfa98xx %s: no file found of type %d\n", __FUNCTION__, type);
...
static enum Tfa98xx_Error tfaContWriteVstep(struct tfa_device *tfa, TfaVolumeStep2File_t *vp, int vstep)
{
	...
	if (tfa->verbose) pr_info("tfa98xx vstep[%d][%d]\n", tfa->dev_idx, vstep);
	...
	struct TfaVolumeStepMessageInfo *old_msg, int enable_partial_update)
{
	...
	if (enable_partial_update) {
		if (new_msg->MessageType != old_msg->MessageType) {
			pr_info("tfa98xx Message type differ - Disable Partial Update\n");
			enable_partial_update = 0;
		}
		else if (tfaContGetmsgLen(new_msg) != tfaContGetmsgLen(old_msg)) {
			pr_info("tfa98xx Message Length differ - Disable Partial Update\n");
			enable_partial_update = 0;
		}
	}
	...
	if (tfa_needs_reset(tfa) == 1) {
		if (new_msg->MessageType == 0) {
			...
			if (tfa->verbose)
				pr_info("tfa98xx P-ID for SetAlgoParams modified!\n");
		}
		else if (new_msg->MessageType == 2) {
			cmdid[2] = SB_PARAM_SET_MBDRC;
			if (tfa->verbose)
				pr_info("tfa98xx P-ID for SetMBDrc modified!\n");
		}
	}
	...
	if (enable_partial_update) {
		...
		if (!partial)
			pr_info("tfa98xx Partial update memory error - Disabling\n");
	}
	...
	if (partial) {
		...
		*p++ = 0x00;
		...
		if (trim == partial) {
			pr_info("tfa98xx No Change in message - discarding %d bytes\n", len);
			len = 0;

		}
		else if (trim < (partial + len - 3)) {
			pr_info("tfa98xx Using partial update: %d -> %d bytes\n", len, (int)(trim - partial + 3));
			...
			pr_info("tfa98xx Partial too big - use regular update\n");
		}
	}
	...
	if (use_partial_coeff) {
		...
		if (tfa->verbose)
			pr_info("tfa98xx Command-ID used: 0x%02x%02x%02x \n", cmdid[0], cmdid[1], cmdid[2]);
...
static enum Tfa98xx_Error tfaContWriteVstepMax2(struct tfa_device *tfa, TfaVolumeStepMax2File_t *vp, int vstep_idx, int vstep_msg_idx)
{
	...
	if (vstep_idx >= vp->NrOfVsteps) {
		pr_info("tfa98xx Volumestep %d is not available \n", vstep_idx);
		return Tfa98xx_Error_Bad_Parameter;
	}

	if (tfa->p_regInfo == NULL) {
		if (tfa->verbose)
			pr_info("tfa98xx Inital vstep write\n");
		enp = 0;
	}
	...
	if (enp) {
		...
		if (nrMessages != p_msgInfo->NrOfMessages) {
			pr_info("tfa98xx Message different - Disable partial update\n");
			enp = 0;
		}
	}
...
#ifdef TFA_DEBUG
	if (tfa->verbose)
		pr_info("tfa98xx bitfield: %s=0x%x (0x%x[%d..%d]=0x%x)\n", tfaContBfName(bfUni.field, tfa->rev), value,
...
static enum Tfa98xx_Error tfaRunWriteDspMem(struct tfa_device *tfa, TfaDspMem_t *cfmem)
{
	...
	for (i = 0; i < cfmem->size; i++) {
		if (tfa->verbose)
			pr_info("tfa98xx dsp mem (%d): 0x%02x=0x%04x\n", cfmem->type, cfmem->address, cfmem->words[i]);
...
static enum Tfa98xx_Error tfaRunWriteFilter(struct tfa_device *tfa, TfaContBiquad_t *bq)
{
	...
	if (tfa->verbose) {
		if (channel == 2)
			pr_info("tfa98xx filter[%d,S]", bq->aa.index);
		else if (channel == 1)
			pr_info("tfa98xx filter[%d,P]", bq->aa.index);
		else
			pr_info("tfa98xx filter[%d]", bq->aa.index);
	}
	...
	for (i = 0; i < runs; i++) {
		...
		if (dmem == Tfa98xx_DMEM_ERR) {
			if (tfa->verbose) {
				pr_info("tfa98xx Warning: XFilter settings are applied via msg file (ini filter[x] format is skipped).\n");
			}
...
static enum Tfa98xx_Error tfaRunWriteRegister(struct tfa_device *tfa, TfaRegpatch_t *reg)
{
	...
	if (tfa->verbose)
		pr_info("tfa98xx register: 0x%02x=0x%04x (msk=0x%04x)\n", reg->address, reg->value, reg->mask);
...
enum Tfa98xx_Error tfaContWriteFiles(struct tfa_device *tfa)
{
	...
	for (i = 0; i < dev->length; i++) {
		...
		if (dev->list[i].type == dscSetInputSelect ||
			...
			if (tfa->verbose) {
				pr_info("tfa98xx command: %s=0x%02x%02x%02x \n",
...
enum Tfa98xx_Error tfa_write_filters(struct tfa_device *tfa, int prof_idx)
{
	...
	if (tfa->verbose) {
		pr_info("tfa98xx ----- profile: %s (%d) -----\n", tfaContGetString(tfa->cnt, &prof->name), prof_idx);
		pr_info("tfa98xx Waiting for CLKS... \n");
	}
...
unsigned int tfa98xx_get_profile_sr(struct tfa_device *tfa, unsigned int prof_idx)
{
	...
	if (tfa->verbose)
		pr_info("tfa98xx %s - profile fs: 0x%x = %dHz (%d - %d)\n",
	...
	if (tfa->verbose)
		pr_info("tfa98xx %s - default fs: 0x%x = %dHz (%d - %d)\n",
			__FUNCTION__, fs_profile,
			tfa98xx_sr_from_field(fs_profile),
			tfa->dev_idx, prof_idx);
...
static enum Tfa98xx_Error get_sample_rate_info(struct tfa_device *tfa, TfaProfileList_t *prof, TfaProfileList_t *previous_prof, int fs_previous_profile)
{
	...
	if (tfa->verbose) {
		pr_info("tfa98xx sample rate from the previous profile: %d \n", fs_previous_profile);
		pr_info("tfa98xx sample rate in the default section: %d \n", fs_default_profile);
		pr_info("tfa98xx sample rate for the next profile: %d \n", fs_next_profile);
	}
	*/

	if (fs_next_profile != fs_default_profile) {
		if (tfa->verbose)
			pr_info("tfa98xx Writing delay tables for AUDFS=%d \n", fs_next_profile);
...
enum Tfa98xx_Error tfaContWriteProfile(struct tfa_device *tfa, int prof_idx, int vstep_idx)
{
	...
	if (prof->group == previous_prof->group && prof->group != 0) {
		if (tfa->verbose) {
			pr_info("tfa98xx The new profile (%s) is in the same group as the current profile (%s) \n",
		...
		if (((tfa->tfa_family == 2) && (manstate >= 6)) || (tfa->tfa_family != 2)) {
			...
			if (tries > 100) {
				pr_info("tfa98xx Wait for PLL powerdown timed out!\n");
				return Tfa98xx_Error_StateTimedOut;
			}
		}
		else {
			pr_info("tfa98xx No need to go to powerdown now \n");
		}
	}
	...
	if (tfa->verbose) {
		pr_info("tfa98xx ---------- default settings profile: %s (%d) ---------- \n",
	...
	if (tfa->verbose)
		pr_info("tfa98xx ---------- new settings profile: %s (%d) ---------- \n",
			tfaContGetString(tfa->cnt, &prof->name), prof_idx);
	...
	if (tfa->ext_dsp != 0) {
			...
			for (i = 0; i < previous_prof->length; i++) {
				...
				if (i < previous_prof->length) {
					if (previous_prof->list[i].type == dscFile || previous_prof->list[i].type == dscPatch) {
						...
						if (tfa->verbose && k == 0) {
							pr_info("tfa98xx ---------- files default profile: %s (%d) ---------- \n",
		...
		if (tfa->verbose) {
			pr_info("tfa98xx ---------- files new profile: %s (%d) ---------- \n",
				tfaContGetString(tfa->cnt, &prof->name), prof_idx);
		}
	}
...
int tfaContGetCalProfile(struct tfa_device *tfa)
{
	...
	for (prof = 0; prof < tfa->cnt->nprof; prof++) {
		if (strstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, prof), ".cal") != NULL) {
			cal_idx = prof;
			pr_info("tfa98xx Using calibration profile: '%s'\n", tfaContProfileName(tfa->cnt, tfa->dev_idx, prof));
			break;
		}
	}
...
int tfa_tib_dsp_msgmulti(struct tfa_device *tfa, int length, const char *buffer)
{
	...
	if (blob == NULL) {
		if (tfa->verbose)
			pr_info("tfa98xx %s, Creating the multi-message \n\n", __FUNCTION__);
