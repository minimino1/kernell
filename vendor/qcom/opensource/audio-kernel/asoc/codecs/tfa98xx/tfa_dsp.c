...
int tfa_irq_report(struct tfa_device *tfa)
{
	...
	{
		if (activemask & (1 << irq))
			pr_err("tfa98xx: device[%d] interrupt: %s %s\n",
...
void tfa_set_query_info(struct tfa_device *tfa)
{
	...
	tfa->supportDrc = supportNotSet;*/
    pr_err("tfa98xx: device type : 0x%02x\n", tfa->rev);
	switch (tfa->rev & 0xff) {
...
enum Tfa98xx_Error tfa98xx_get_mtp(struct tfa_device *tfa, uint16_t *value)
{
	...
	if(tfa->tfa_family == 1)
	{
		...
		if (TFA_GET_BF(tfa, PWDN)) {
			pr_info("tfa98xx PLL in powerdown\n");
			return Tfa98xx_Error_NoClock;
		}

		tfa98xx_dsp_system_stable(tfa, &status);
		if (status == 0) {
			pr_info("tfa98xx PLL not running\n");
			return Tfa98xx_Error_NoClock;
		}
	}
...
void tfa2_manual_mtp_cpy(struct tfa_device *tfa, uint16_t reg_row_to_keep, uint16_t reg_row_to_set, uint8_t row)///MCH_TO_TEST
{
	...
	if (tfa->is_probus_device)
	{
		...
		if (tfa->verbose) {
			pr_info("tfa98xx FAIM enabled (err:%d).\n", error);
		}
	}
	...
	if (tfa->is_probus_device)
	{
		...
		if (tfa->verbose) {
			pr_info("tfa98xx FAIM disabled (err:%d).\n", error);
		}
	}
}
...
enum Tfa98xx_Error tfa98xx_set_mtp(struct tfa_device *tfa, uint16_t value, uint16_t mask)
{
	...
	if (mtp_old == mtp_new) /* no change */ {
		if (tfa->verbose)
			pr_info("tfa98xx: No change in MTP. Value not written! \n");
		return Tfa98xx_Error_Ok;
	}
	...
	if (tfa->verbose) {
		pr_info("tfa98xx MTP clock enabled.\n");
	}
	...
	if (tfa->verbose) {
		pr_info("tfa98xx MTP clock disabled.\n");
	}
...
void tfa98xx_set_exttemp(struct tfa_device *tfa, short ext_temp)
{
	if ((-256 <= ext_temp) && (ext_temp <= 255)) {
		...
		pr_info("tfa98xx Using ext temp %d C\n", twos(ext_temp));
		TFA_SET_BF(tfa, TROS, 1);
		TFA_SET_BF(tfa, EXTTS, twos(ext_temp));
	}
	else {
		pr_info("tfa98xx Clearing ext temp settings\n");
...
tfa98xx_check_ic_rom_version(struct tfa_device *tfa, const unsigned char patchheader[])
{
	...
	if (checkaddress != 0xFFFF) {
		...
		if (error == Tfa98xx_Error_Ok) {
			if (value != checkvalue) {
				pr_err("tfa98xx: patch file romid mismatch [0x%04x]: expected 0x%02x, actual 0x%02x\n",
	 ...
	 /* check if the revid subtype is in there */
		if (checkvalue != 0xFFFFFF && checkvalue != 0) {
			...
			if (revid != tfa->rev) {
				if ((tfa->revid != 0) && (revid != tfa->revid)) {
					pr_err("tfa98xx: container patch and HW mismatch: expected: 0x%02x, actual 0x%02x\n",
						tfa->revid, revid);
					return Tfa98xx_Error_Not_Supported;
				}
				pr_err("tfa98xx: container patch and HW mismatch: expected: 0x%02x, actual 0x%02x\n",
...
enum Tfa98xx_Error tfa_dsp_msg(struct tfa_device *tfa, int length24, const char *buf24)
{
	...
	if (tfa->ext_dsp == 1) {
		...
		if (error == Tfa98xx_Error_Buffer_too_small) {
			...
			if (tfa->verbose) {
				pr_info("tfa98xx Multi-message buffer full. Sending multi-message, length=%d \n", len);
			}
		...
		if (lastmessage == 1) {
			...
			if (tfa->verbose)
				pr_info("tfa98xx Last message for the multi-message received. Multi-message length=%d \n", length);
	...
	if ((tfa->verbose & 0x04) != 0) {
		pr_info("tfa98xx DSP w [%d]: ", length);
		for (i = 0; i < length; i++)
			pr_info("tfa98xx 0x%02x ", (uint8_t)buf[i]);
		pr_info("tfa98xx \n");
	}
...
enum Tfa98xx_Error tfa_dsp_msg_read(struct tfa_device *tfa, int length24, unsigned char *bytes24)
{
	...
	/* DSP verbose has argument 0x04 */
	if ((tfa->verbose & 0x04) != 0) {
		pr_info("tfa98xx DSP R [%d]: ", length);
		for (i = 0; i < length; i++)
			pr_info("tfa98xx 0x%02x ", (uint8_t)bytes[i]);
		pr_info("tfa98xx \n");
	}
	...
	unsigned char data[])
{
	...
	if (num_bytes <= 0) {
		pr_info("tfa98xx Error: The number of READ bytes is smaller or equal to 0! \n");
		return Tfa98xx_Error_Fail;
	}
...
int tfa_set_bf(struct tfa_device *tfa, const uint16_t bf, const uint16_t value)
{
	...
	if (err) {
		pr_err("tfa98xx: Error getting bf :%d \n", -err);
		return -err;
	}
	...
	if (oldvalue != regvalue) {
		...
		if (err) {
			pr_err("tfa98xx: Error setting bf :%d \n", -err);
			return -err;
		}
	}
...
enum Tfa98xx_Error tfa_cf_powerup(struct tfa_device *tfa)
{
	...
	if (tfa->verbose)
		pr_info("tfa98xx: Waiting for DSP system stable...\n");
	...
	if (tries == 0) {// timedout
		pr_err("tfa98xx: DSP subsystem start timed out\n");
...
static enum Tfa98xx_Error tfa98xx_aec_output(struct tfa_device *tfa, int enable)
{
	...
	if (tfa->tfa_family == 1)
		...
		pr_err("tfa98xx: I2SDOE on unsupported family\n");
...
enum Tfa98xx_Error tfa_show_current_state(struct tfa_device *tfa)
{
	...
	if (tfa->tfa_family == 2 && tfa->verbose) {
		...
		pr_info("tfa98xx Current HW manager state: ");

		switch (manstate) {
		case 0: pr_info("tfa98xx power_down_state \n");
			break;
		case 1: pr_info("tfa98xx wait_for_source_settings_state \n");
			break;
		case 2: pr_info("tfa98xx connnect_pll_input_state \n");
			break;
		case 3: pr_info("tfa98xx disconnect_pll_input_state \n");
			break;
		case 4: pr_info("tfa98xx enable_pll_state \n");
			break;
		case 5: pr_info("tfa98xx enable_cgu_state \n");
			break;
		case 6: pr_info("tfa98xx init_cf_state \n");
			break;
		case 7: pr_info("tfa98xx enable_amplifier_state \n");
			break;
		case 8: pr_info("tfa98xx alarm_state \n");
			break;
		case 9: pr_info("tfa98xx operating_state \n");
			break;
		case 10: pr_info("tfa98xx mute_audio_state \n");
			break;
		case 11: pr_info("tfa98xx disable_cgu_pll_state \n");
			break;
		default:
			pr_info("tfa98xx Unable to find current state \n");
			break;
		}
	}
...
enum Tfa98xx_Error tfaGetFwApiVersion(struct tfa_device *tfa, unsigned char *pFirmwareVersion)
{
	...
	if (!tfa->is_probus_device)
	{
		...
		if (err) {
			pr_info("tfa98xx %s Error: Unable to get API Version from DSP \n", __FUNCTION__);
			return err;
		}
...
enum Tfa98xx_Error tfaRunSpeakerStartup(struct tfa_device *tfa, int force, int profile)
{
	...
	if (err) {
		pr_info("tfa98xx [%s] cannot get FWAPI error = %d \n", __FUNCTION__, err);
		return err;
	}
	/* write all the files from the device list */
	err = tfaContWriteFiles(tfa);
	if (err) {
		pr_info("tfa98xx [%s] tfaContWriteFiles error = %d \n", __FUNCTION__, err);
		return err;
	}
	...
	if (err) {
		pr_info("tfa98xx [%s] tfaContWriteFilesProf error = %d \n", __FUNCTION__, err);
		return err;
	}
...
#define CF_CONTROL 0x8100
	...
	while (state != TFA_GET_BF(tfa, ACS)) {
		...
		if (tries-- == 0) {
			pr_info("tfa98xx coldboot (ACS) did not %s\n", state ? "set" : "clear");
...
enum Tfa98xx_Error tfaRunStartup(struct tfa_device *tfa, int profile)
{
	...
	if ((tfa->dynamicTDMmode == 3) && tfa_dev_set_tdm_bitwidth(tfa,tfa->bitwidth))
		...
		pr_info("\ntfa98xx Warning: No init keyword found in the cnt file. Init is skipped! \n");
	}
...
static void individual_calibration_results(struct tfa_device *tfa)
{
	...
	if (value_P != 1 && value_S != 1)
		pr_info("tfa98xx Calibration failed on both channels! \n");
	else if (value_P != 1) {
		pr_info("tfa98xx Calibration failed on Primary (Left) channel! \n");
		TFA_SET_BF_VOLATILE(tfa, SSLEFTE, 0); /* Disable the sound for the left speaker */
	}
	else if (value_S != 1) {
		pr_info("tfa98xx Calibration failed on Secondary (Right) channel! \n");
	...
	*calibrateDone = 0;
	...
	if (TFA_GET_BF(tfa, MTPOTC)) {
		...
		if (tries_mtp_busy < MTPBWAIT_TRIES) {
			...
			pr_err("tfa98xx MTP bussy after %d tries\n", MTPBWAIT_TRIES);
		}
	}
	...
	if (*calibrateDone != 1) {
		...
		pr_info("tfa98xx Calibration has timedout! \n");
		err = Tfa98xx_Error_StateTimedOut;
	}
	else if (tries_mtp_busy == 1000) {
		pr_err("tfa98xx Calibrate Failed: MTP_busy stays high! \n");
		err = Tfa98xx_Error_StateTimedOut;
	}
	...
	if (err != Tfa98xx_Error_Ok) {
		if ((tfa->tfa_family == 2) && (TFA_GET_BF(tfa, REFCKSEL) == 1)) {
			pr_err("tfa98xx Unable to calibrate the device with the internal clock! \n");
		}
	}
...
enum tfa_error tfa_dev_start(struct tfa_device *tfa, int next_profile, int vstep)
{
	...
	pr_info("tfa98xx Active_profile:%s, next_profile:%s\n",
	...
	if (tfa->bus != 0) { /* non i2c  */
		...
		if (strstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, next_profile), ".standby") != NULL) {
			pr_info("tfa98xx Skip powering on device, in standby profile!\n");
		} else {
	...
	if (strstr(tfaContProfileName(tfa->cnt, tfa->dev_idx, next_profile), ".standby") != NULL) {
		...
		pr_info("tfa98xx Power down device, by force, in standby profile!\n");
    ...
    if ((tfa->rev & 0xff) == 0x66 &&
        tfa_get_bf(tfa, TFA986X_BF_DUALCELL)) {
        uint16_t dualcell_trims[16] = { 9, 9, 9, 9,
                                        9, 9, 9, 9,
                                        9, 9, 9, 9,
                                        9, 9, 9, 9};
        uint16_t defaults = tfa_get_bf(tfa, 0xf053);
        tfa_set_bf(tfa, 0xf053, dualcell_trims[defaults]);
        pr_info("%s: set trim from %d to %d\n", __func__, defaults, tfa_get_bf(tfa, 0xf053));
    }
...
enum tfa_error tfa_dev_stop(struct tfa_device *tfa)
{
	...
	if((tfa->tfa_family==2)&&(tfa->is_probus_device==0)) {
		...
		while((manstate >= 4) && (tries < 100)) {
			pr_info("tfa98xx Waiting for state machine to powerdown, MANSTATE = %d\n", manstate);
			msleep_interruptible(10);
			manstate = TFA_GET_BF(tfa, MANSTATE);
			tries++;
		}
		pr_info("tfa98xx exit.... MANSTATE = %d\n", manstate);
	} else if (tfa->rev == 0x1b96) {
        temps = tfa_get_bf(tfa, TFA9896_BF_TEMPS);
        while((temps != 0x100) && (tries < 100)) {
            pr_err("tfa98xx Waiting for state machine to powerdown, TEMPS = %d, tries=%d\n", temps, tries);
            msleep_interruptible(10);
            temps = tfa_get_bf(tfa, TFA9896_BF_TEMPS);
            tries++;
        }
        pr_info("tfa98xx exit, TEMPS = %d, tries=%d\n", temps, tries);
    }
...
int tfa_reset(struct tfa_device *tfa)
{
	...
	if (tfa->verbose) {
		if (((tfa->tfa_family == 1) && state != TFA_STATE_RESET) ||
			...
			pr_info("tfa98xx WARNING: Device reset should be performed in POWERDOWN state\n");
		}
	}
...
enum Tfa98xx_Error tfa_dsp_partial_coefficients(struct tfa_device *tfa, uint8_t *prev, uint8_t *next)
{
	...
	for (eq = 0; eq < NR_EQ; eq++) {
		...
		pr_info("tfa98xx eq_biquad_mask[%d] = 0x%.8x\n", eq, eq_biquad_mask[eq]);

		eq_offset += eq_biquads[eq];
	}

	pr_info("tfa98xx cost for writing all coefficients     = %d\n", old_cost);
	pr_info("tfa98xx cost for writing changed coefficients = %d\n", new_cost);
...
enum tfa_error tfa_dev_set_state(struct tfa_device *tfa, enum tfa_state state, int is_calibration)
{
	...
	case TFA_STATE_OPERATING:    /* Amp and Algo running */
		...
		if (tfa->sync_iv_delay) {
			if (tfa->verbose)
				pr_info("tfa98xx syncing I/V delay for %x\n",
				(tfa->rev & 0xff));
...
int tfa_dev_mtp_get(struct tfa_device *tfa, enum tfa_mtp item)
{
	...
	case TFA_MTP_RE25_SEC:
		if ((tfa->rev & 0xFF) == 0x88) {
			...
			pr_info("tfa98xx Error: Current device has no secondary Re25 channel \n");
		}
		break;
	case TFA_MTP_LOCK:
		break;
...
enum Tfa98xx_Error tfa_status(struct tfa_device *tfa)
{
	...
	/* pr_info("tfa98xx SYS_STATUS0: 0x%04x\n", val); */
...
int tfa_wait4manstate(struct tfa_device *tfa, uint16_t bf, uint16_t wait_value, int loop)
{
	...
	if ( rc == -ETIME )
		pr_err("tfa98xx timeout waiting for bitfield:0x%04x, value:%d, %d times\n",
...
void tfa_dev_get_tdm_add(struct tfa_device* tfa, uint16_t* tdme, uint16_t* tnbck, uint16_t* tslln, uint16_t* tsize)
{
	...
	default:
		pr_err("tfa98xx unsuported device rev:%x\n", revid);
		break;
	}
}
int tfa_dev_set_tdm_bitwidth(struct tfa_device* tfa, int width)/*SACAS-26*/
{
	...
	case 32: /* 32-bit sample in 32-bit slot */
		...
		break;
	default:
		pr_err("tfa98xx unsupported tdm bitwidth:%d\n", width);
		return -EINVAL;
		break;
	}
...
void tfa_adapt_noisemode(struct tfa_device *tfa)
{
	...
	{
		if (tfa->verbose)
			pr_info("tfa98xx Adapting low noise mode is not needed, condition not fulfilled!\n");
		return;
	}
	if (tfa->verbose)
		pr_info("tfa98xx Adapting low noise mode\n");

	for (i = 0; i < NR_OF_BATS; i++) {
		int bat = TFA_GET_BF(tfa, BATS);
		if (tfa->verbose)
			pr_info("tfa98xx bats[%d]=%d\n", i, bat);
		total_bats += bat;
