...
static enum Tfa98xx_Error tfa9912_specific(struct tfa_device *tfa)
{
	...
	if (tfa->rev == 0x1a13) {
		...
		pr_info("tfa98xx Warning: Optimal settings not found for device with revid = 0x%x \n", tfa->rev);
	}
	...
	if (result) {
		...
		if (result == 0) {
			...
			if (currentValue + delta < 15) {
				...
				if (tfa->verbose)
					pr_info("tfa98xx Max coil current is set to: %d \n", currentValue + delta);
			}
			else {
				TFA_SET_BF_VOLATILE(tfa, DCMCC, 15);
				if (tfa->verbose)
					pr_info("tfa98xx Max coil current is set to: 15 \n");
			}
		}
		else if (result == 1) {
			...
			if (currentValue - delta > 0) {
				...
				if (tfa->verbose)
					pr_info("tfa98xx Max coil current is set to: 0 \n");
			}
		}
	}
...
static uint16_t tfa9912_vboost_fixup(struct tfa_device *tfa, uint16_t dcdc_cnt6)
{
	...
	if (new_dcdc_cnt6 != dcdc_cnt6) {
		if (tfa->verbose)
			pr_info("tfa98xx tfa9912: V boost fixup applied. Old 0x%04x, new 0x%04x\n",
...
static enum Tfa98xx_Error tfa9872_specific(struct tfa_device *tfa)
{
	...
	case 0x3b72:
		...
		break;
	default:
		pr_info("\ntfa98xx: Warning: Optimal settings not found for device with revid = 0x%x \n", tfa->rev);
		break;
	}
...
static enum Tfa98xx_Error tfa9873_specific(struct tfa_device* tfa)
{
	...
	pr_info("tfa98xx: info : disabled FS synchronisation! \n");
	return error;
}
...
enum Tfa98xx_Error tfa9875_tfa_status(struct tfa_device* tfa)
{
	...
	if (!tfa_get_bf(tfa, TFA9875_BF_CLKS))
		pr_err("tfa98xx: ERROR: CLKS is unstable\n");
	if (!tfa_get_bf(tfa, TFA9875_BF_PLLS))
		pr_err("tfa98xx: ERROR: PLL not locked\n");
	...
	{
		pr_err("tfa98xx: BODNOK error detected : STATUS_FLAG3 = 0x%x\n", (uint16_t)tfa_read_reg(tfa, TFA9875_BF_BODNOK));
...
static enum Tfa98xx_Error tfa986x_specific(struct tfa_device *tfa)
{
    ...
    if (rc < 0) {
        pr_err("tfa98xx: Error, waiting powerdown leaving\n");
        return rc;
    }
	...
	if (rc < 0)
	{
		pr_err("tfa98xx: Timeout waiting for manstate 0\n");
		return rc;
	}
...
static enum Tfa98xx_Error tfa9894_specific(struct tfa_device *tfa)
{
	...
	xor = value ^ 0x005A;
	error = tfa_reg_write(tfa, 0xA0, xor);
	pr_info("tfa98xx Device REFID:%x\n", tfa->rev);
	/* The optimal settings */
	if (tfa->rev == 0x0a94) {
		/* V36 */
		/* ----- generated code start ----- */
